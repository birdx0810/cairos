    ...
    <div class="bg-white/80 backdrop-blur rounded-lg p-4 border border-gray-200/50 shadow-sm h-full">
      <div class="flex justify-between items-center mb-4">
        ...
        <div class="flex gap-2">
          <button
            @click="clearSchedule"
            class="px-3 py-1 text-sm bg-red-500 text-white rounded hover:bg-red-600 transition-colors flex items-center gap-1"
            :disabled="!hasContent"
          >
            <span>Clear All</span>
          </button>
          <button
            @click="exportToExcel"
            class="px-3 py-1 text-sm bg-green-500 text-white rounded hover:bg-green-600 transition-colors flex items-center gap-1"
...
// Computed property to check if any schedule items have content
const hasContent = computed(() => {
  return props.scheduleData.some(item => item.plan || item.outcome)
})

// Method to clear all plans and outcomes
const clearSchedule = () => {
